...
VALID_SIGNALS = {"BUY","SELL","BUY (REVERSAL)","BUY (SETUP)","SELL (SETUP)"}

MIN_CONF = {
    "BUY": 35, "SELL": 25,
    "BUY (SETUP)": 30, "SELL (SETUP)": 30,
    "BUY (REVERSAL)": 40,
}
...
def _analyse_single(symbol: str, timeframe: str, min_score: float = 0) -> Optional[dict]:
    try:
        ...
        try:
            ...
            regime_min = {"TRENDING":35,"BREAKOUT":38,"NEUTRAL":40,"RANGING":50,"VOLATILE":60}.get(reg, 40)
            if confidence < regime_min:
                return None
            if reg == "TRENDING":
                if signal.startswith("BUY")  and not regime.get("bias_buy"):  return None
                if signal.startswith("SELL") and not regime.get("bias_sell"): return None
        except Exception as e:
        ...
        try:
            ...
            _xgb_ok, _xgb_prob, _xgb_reason = get_trainer("virtual_trading.db").should_entry(
