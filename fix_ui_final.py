...
old = """        text = text_raw.lower()
        ...
        # Handle pending action (menu yang butuh input lanjutan)
        if chat_id in _pending_action:
            action = _pending_action.pop(chat_id)
            if action == "analyze":
                text = f"/analyze {text_raw}"
            elif action == "execute":
                text = f"/execute {text_raw}"
...
if old in content:
    ...
    with open('telegram_sender.py', 'w', encoding='utf-8') as f:
        f.write(content)
    print("SUCCESS: Urutan handler diperbaiki")
else:
    print("NOT_FOUND: Pattern tidak cocok, perlu cek manual")
