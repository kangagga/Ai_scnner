...
def _detect_regime_from_row(row) -> str:
    """
    Replika logic detect_market_regime() dari market_context.py,
    tapi dihitung dari 1 row historis (bukan fetch data live).
    """
    ...
    bb_middle = float(row.get("bb_mid", price))
...
def run_backtest(
    ...
    for i in range(len(df_test) - 1):
        ...
        # Filter [ALIGNED 2026-07-04] — samakan urutan & logic dengan scanner.py
        if signal not in VALID_SIGNALS:
            prev_signal = signal
            continue
        if conf < MIN_CONF.get(signal, min_confidence):
            prev_signal = signal
            continue

        # Regime filter — replika scanner.py: blokir NEUTRAL/VOLATILE/UNKNOWN
        reg = _detect_regime_from_row(row)
        if reg in ("NEUTRAL", "VOLATILE", "UNKNOWN"):
            prev_signal = signal
            continue
        regime_min_conf = {"TRENDING": 50, "BREAKOUT": 55, "RANGING": 48}.get(reg, 50)
        if conf < regime_min_conf:
            prev_signal = signal
            continue
        ...
        # [ALIGNED 2026-07-04] Simulasi exit replikasi trailing logic exit_monitor.py:
        # TP1 hit -> SL geser ke entry, lanjut pantau TP2
        # TP2 hit -> SL geser ke TP1 original, lanjut pantau TP3
        # TP3 hit atau SL (termasuk SL yang sudah ditrailing) -> exit total
        exit_price  = None
        exit_reason = "OPEN"
        exit_candle = None
        pnl_pct     = 0.0
        ...
        for j in range(i + 1, min(i + 50, len(df_test))):   # max 50 candle ke depan
            ...
            if direction == "BUY":
                # Cek SL dulu (prioritas keamanan)
                if low <= cur_sl:
                    exit_price  = cur_sl
                    exit_reason = "SL" if monitoring_tp == "TP1" else "TRAILING STOP"
                    exit_candle = j - i
                    break
                # Cek TP sesuai tahap monitoring saat ini
                if monitoring_tp == "TP1" and high >= tp1:
                    cur_sl = entry           # SL geser ke entry
                    monitoring_tp = "TP2"
                    continue
                elif monitoring_tp == "TP2" and high >= tp2:
                    cur_sl = tp1             # SL geser ke TP1 original
                    monitoring_tp = "TP3"
                    continue
                elif monitoring_tp == "TP3" and high >= tp3:
                    exit_price  = tp3
                    exit_reason = "TP3"
                    exit_candle = j - i
                    break
                ...
                if high >= cur_sl:
                    ...
                    exit_candle = j - i
                    break
                if monitoring_tp == "TP1" and low <= tp1:
                    cur_sl = entry
                    monitoring_tp = "TP2"
                    continue
                elif monitoring_tp == "TP2" and low <= tp2:
                    cur_sl = tp1
                    monitoring_tp = "TP3"
                    continue
                elif monitoring_tp == "TP3" and low <= tp3:
                    exit_price  = tp3
                    exit_reason = "TP3"
                    exit_candle = j - i
                    break
        ...
        trades.append({
            ...
            "win"         : exit_reason in ("TP1", "TP2", "TP3", "TRAILING STOP"),
        })
