...
def _analyse_single(symbol, timeframe, min_score=0):
    try:  # outer guard — error 1 pair tidak stop bot
        ...
        # ── SR GUARD: Sinyal harus searah support/resistance ──
        if support > 0 and resistance > 0 and entry > 0:
            sr_range  = resistance - support
            if sr_range > 0:
                pos_pct   = (entry - support) / sr_range  # 0=di support, 1=di resistance
                near_sup  = pos_pct < 0.20   # dalam 20% bawah range = zona support
                near_res  = pos_pct > 0.80   # dalam 20% atas range = zona resistance

                if signal.startswith("SELL") and near_sup:
                    logger.info(f"[SR_GUARD] {symbol}/{timeframe}: SELL diblok — harga di zona support {support:.8f} (pos={pos_pct:.0%})")
                    return None

                if signal.startswith("BUY") and near_res:
                    logger.info(f"[SR_GUARD] {symbol}/{timeframe}: BUY diblok — harga di zona resistance {resistance:.8f} (pos={pos_pct:.0%})")
                    return None
        ...
        if SMC_AVAILABLE:
            try:
                smc_data   = smc_confidence(df, signal=signal_type)
                smc_bonus  = smc_data.get("score_adjustment", 0)
                smc_report = format_smc_report(smc_data, symbol, signal_type)
            except Exception as _e:
        ...
        if OB_AVAILABLE:
            try:
                ...
                ob_bonus    = ob_score_adjustment(ob_features, signal_type)
        ...
        logger.error(f"[ERROR] _analyse_single {symbol}/{timeframe}: {_e}", exc_info=True)
        return None
...
def scan_all_fast(symbols=None, timeframe="all", min_score=0):
    ...
    if symbols is None:
        try:
            ...
            LEVERAGED_SUFFIXES = ("3L", "5L", "3S", "5S")
            seen, symbols = set(), []
            for s in combined:
                if s not in seen:
                    seen.add(s)
